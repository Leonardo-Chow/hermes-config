...
1. **❌ OBSBOT 官方账号视频**（2026-08-06 用户明确要求）：官方频道发布的产品宣传/教程视频不纳入报告，如 "Flip to Meet"、"How to Use XXX"、官方促销等
2. **❌ 时长超过1小时的视频**（2026-08-26 用户明确要求）：直播回放/长视频自动过滤，如 Michael Alama 的长期直播、Park Pass TV 的迪士尼直播等
3. **❌ AI配音的视频**（2026-08-31 用户明确要求）：原声是 AI 配音（TTS 机器音）的视频直接过滤。常见特征：无真人出镜、机械语音旁白、批量生成的"Top 5/榜单"类内容、画质廉价感
4. **设备列表/产品列表视频**：提到OBSBOT但无实质评测内容
3. **游戏/直播/杂谈**：OBSBOT仅作为"设备之一"被提及
4. **技术参数对比/规格表**：无实际使用体验
5. **专业媒体/技术评测**：GadgetDoc等技术分析型频道，非KOC视角
6. **Vlog/直播中的提及**：在其他内容（赛车、游戏）中顺便使用OBSBOT
7. **俄语/日语/韩语/东南亚语言博主**：自动过滤（俄语KOL自发评测/开箱/对比等有价值内容应保留）
