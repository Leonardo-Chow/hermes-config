...
**🚨 AI 配音直接 PASS（2026-08-31 用户确认）：**
- 视频原声是 **AI 配音**（TTS 合成音、机器朗读、无真人声音）→ **直接过滤，不纳入报告**
- 判断方法：打开视频听原声，若为明显的 TTS 电子音/无感情机器朗读 → 排除
...
- ❌ **时长 > 3600秒（1小时）→ 直接过滤**（2026-08-26 新增，直播/超长视频不属于竞品投放监测范围）
  - ⚠️ 注意：直播回放、整场教会直播、超长 setup 视频（>1h）一律过滤
  - 案例：PUBGPROISLIVE 258min、Thiago Gomes 72min、First Baptist Church 80min+ → 均不再纳入
