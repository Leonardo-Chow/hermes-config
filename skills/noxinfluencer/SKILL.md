...
| Tier | YouTube Avg Views | YouTube Followers | Typical Collab Cost |
|------|-------------------|-------------------|-------------------|
| **Nano** | < 10k | < 30k | $50 - $200 |
| **Mid-tier** | 10k - 30k | 30k - 100k | $200 - $500 |
| **Lower Macro** | 30k - 50k | 100k - 200k | $400 - $900 |
| **Macro** | 50k - 100k | 200k - 500k | $800 - $2,000 |
| **Elite** | ≥ 100k | ≥ 500k | $2,000+ |

**Nano/Mid-tier targeted search pattern:**
```bash
noxinfluencer creator search --platform youtube \
  --keywords '[<niche>,<sub-niche>]' \
  --country '[US,CA]' \
  --avg_view_min 3000 \
  --avg_view_max 50000 \
  --follower_min 3000 \
...
**Key filters for mid-tier/nano:**
- `--avg_view_min 3000` — skip channels with almost no viewership
- `--avg_view_max 40000` (or 50000) — cap to avoid macro/elite
- `--follower_max 150000` (or lower) — additional size guard
- `--published_within_days 90` — recent activity filter (3 months)
- `--follower_min 3000` — minimum follower threshold

### Multi-Niche Batch Search Strategy

When the task requires diverse category coverage, run **parallel searches by niche** rather than one broad search. Example search portfolio for a webcam/PTZ camera product:

```
# Search 1: Tech/3C/gadget reviewers
noxinfluencer creator search --keywords '[webcam review,tech gadget,3C,camera review]' ...

# Search 2: Livestream/streaming gear (core PTZ audience)
noxinfluencer creator search --keywords '[live streaming gear,stream setup,OBS tutorial,PTZ camera]' ...

# Search 3: Desk setup / Content Creator workspace
noxinfluencer creator search --keywords '[desk setup,studio tour,home office,workspace]' ...

# Search 4: Fitness/Yoga (movement tracking use case)
noxinfluencer creator search --keywords '[fitness training,home gym,yoga instructor,workout video]' ...

# Search 5: Music production / recording studio
noxinfluencer creator search --keywords '[music production,home studio,recording,instrument tutorial]' ...

# Search 6: Apple ecosystem / accessories
noxinfluencer creator search --keywords '[apple accessories,mac setup,iphone accessories]' ...
```

Each search typically returns 15-20 results per page. Combine results across niches, then deduplicate and apply exclusion list.

### Exclusion Workflow

After collecting NoxInfluencer results, cross-reference against an external collaboration exclusion list:

```python
# Read NoxInfluencer results and cross-reference with collab list
import openpyxl
collab_wb = openpyxl.load_workbook('/path/to/collaborations.xlsx')
collab_set = set()
for row in collab_wb['Sheet1'].iter_rows(min_row=3, values_only=True):
    name = str(row[2]).strip() if row[2] else ''
    if name: collab_set.add(name.lower())

# Check each NoxInfluencer candidate
for candidate in candidates:
    if candidate.lower() in collab_set:
        print(f'EXCLUDED: {candidate}')
    else:
        print(f'CLEAR: {candidate}')
```

The exclusion check MUST be case-insensitive. Some collab lists use partial or different names — also check for channel name variants.
...
### ⚠️ Exact-KOL Compliance (User Preference)

**When the user provides specific KOL names they want added to a table/list, add THOSE EXACT names — do not search for or suggest "similar types" as substitutes.** 

If the user says "find these specific ones: [names]", the correct action is:
1. Look up each named KOL in NoxInfluencer or the source template
2. Add them to the output with their data completed
3. Do NOT search for alternative/similar creators unless the user explicitly says "find more like these"
...
### ⚡ GFW Pitfall: NoxInfluencer API Blocked

NoxInfluencer's API server (`skill.noxinfluencer.com`) is behind the GFW. Symptoms:
- `Error: Request failed: fetch failed` (Doctor shows `server_reachable: fail`)
- `HTTP 403 Cloudflare block` (even with proper User-Agent)
...
When mcporter returns `TypeError: fetch failed / ECONNRESET` for `docs.qq.com` but `curl https://docs.qq.com` returns HTTP 200, the issue is usually **expired mcporter token**, not network. Fix:
```bash
...
- `{baseDir}/references/kcl-product-scenario-mapping.md` — KOL tier definitions, multi-niche parallel search templates, and product-feature→KOL-category mapping for PTZ/webcam products
- `{baseDir}/references/obsbot-kol-sourcing-workflow.md` — OBSBOT-specific KOL sourcing workflow: template format, exclusion list, category priorities, file management
