...
?video_url=VIDEO_URL&format=text&include_timestamp=true&send_metadata=true" \
  -H "Authorization: Bearer $TRANS...KEY" \
...
**Response** (`format=text`): ⚠️ `transcript` field is a **string** (not array) with embedded timestamps:

```
[0.32s] Hey chicos, gracias por estar una vez
[1.68s] más en este canal. Mi nombre es Daniel
```

When parsing programmatically, check `type(transcript)` — if string, split by newlines; if array, iterate objects.
...
## YouTube Comment Extraction (via Browser)

When you need to extract comments from a YouTube video page, use browser tools with JavaScript:

```javascript
// After navigating to the video and scrolling down to load comments
const comments = [];
const commentElements = document.querySelectorAll('ytd-comment-thread-renderer');
commentElements.forEach((el, i) => {
    if (i < 10) { // limit to top N comments
...
**Steps:**
1. `browser_navigate` to the YouTube video URL
2. `browser_scroll` down 2-3 times to trigger lazy-loading of comments
3. `browser_console` with the JavaScript above to extract comments
4. Comments section header shows count (e.g., "307 条评论")

**Pitfalls:**
- Comments only load after scrolling past the video description
- YouTube may block bot detection without residential proxies
- `#vote-count-middle` may be empty for comments with few likes

## Getting Video Metadata via TranscriptAPI

To get just the title/tags without full transcript, use `send_metadata=true`:
...
Response includes `metadata.title`, `metadata.author_name`, `metadata.author_url`.
