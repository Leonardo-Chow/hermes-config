...
- **⚠️ 批量上传批次大小**：每批 ≤10 条记录（50 条会导致 mcporter 输出截断至 20K 字符，JSON 解析失败）
- **⚠️ JSON 大载荷**：用 `write_file` 写入临时文件 + `$(cat /tmp/file.json)` 命令替换传递，不要内联 JSON
- **⚠️ 中文标题创建失败**：`manage.create_file` 中文标题可能报错 400010，先用英文标题创建再 `rename_file_title`
- **⚠️ 列重命名**：用 `smartsheet.update_fields` + field_id + 新标题 + 原 field_type
- **⚠️ 批量删除**：用 `python3 -c "import sys,json; ..."` 管道提取 record_id，避免输出截断
