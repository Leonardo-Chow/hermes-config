...
当 `upload_image` 不可用时，用 python-docx 创建 .docx 嵌入图片，再通过文件导入通道上传。

```
1. 用 python-docx 创建文档，直接嵌入本地图片：
   from docx import Document
   from docx.shared import Inches
   doc = Document()
   doc.add_heading('标题', level=1)
   doc.add_picture('/tmp/image.jpg', width=Inches(5.5))
   doc.save('/tmp/output.docx')

2. 图片预处理（macOS 压缩 PNG→JPEG）：
   sips -s format jpeg -s formatOptions 50 input.png --out output.jpg

3. 调用 import_file.sh 上传：
   cd ~/.hermes/skills/tencent-docs && bash import_file.sh /tmp/output.docx
   → 输出 FILE_KEY, FILE_NAME, FILE_MD5, FILE_SIZE, TASK_ID

4. 调用 manage.async_import 触发导入：
   mcporter call tencent-docs manage.async_import --args '{
     "task_id": "<TASK_ID>", "file_size": "<FILE_SIZE>",
     "file_key": "<FILE_KEY>", "file_name": "<FILE_NAME>", "file_md5": "<FILE_MD5>"
   }'

5. 验证（manage.import_progress 不可用，用 search_file 代替）：
   mcporter call tencent-docs manage.search_file --args '{"search_key": "<文档标题>"}'
   → 确认 file_id 和 url 存在即导入成功

6. 移动到目标文件夹：
   mcporter call tencent-docs manage.move_file --args '{"file_id": "<file_id>", "target_folder_id": "<folder_id>"}'
```

> ⚠️ 文件大小建议 < 2MB。大文件 COS 上传会超时。先压缩图片再嵌入。
