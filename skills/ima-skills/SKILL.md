...
> 📖 **Setup guide** with full installation steps, credential configuration, and connection verification is available in `references/setup-guide.md`.
...
```bash
# Step 1: 获取 notebook_id
node ima_api.cjs "openapi/wiki/v1/get_media_info" '{"media_id": "<media_id>"}'
# 从 data.notebook_ext_info.notebook_id 提取

# Step 2: 用 notebook_id 读取
node ima_api.cjs "openapi/note/v1/get_doc_content" '{"note_id": "<notebook_id>"}'
```
如果仍然失败，说明确实无权访问，需告知用户。

### 🔴 import_doc 内容过长时的分段策略
经测试，13888 字符的笔记内容通过 `import_doc` 上传成功。如果仍担心超限：
1. 将内容分为多个部分生成（如 Part 1-3）
2. 合并时去除重复的封面图片和标题
3. 检查 `🕵️` 关键字确认娱乐八卦未被合并逻辑误删
4. 一次性用 `import_doc` 上传完整内容

### 🔴 add_knowledge 中 media_type=11 表示笔记类型
上传文件到知识库与将笔记关联到知识库使用不同的 media_type。笔记关联到知识库必须使用 `media_type=11` 和 `note_info.content_id=<note_id>`。

### 🔴 封面图片 URL 会过期
IMA COS 签名 URL 有时效性。每次生成日报前必须调用 `get_media_info` 获取最新签名 URL，不可缓存使用。
