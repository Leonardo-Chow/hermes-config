...
## Hermes Agent 安装与使用指南

**安装方式**：通过 `skill_view('guizang-ppt-skill')` 加载。

**手动安装（从 GitHub 下载）：**
```bash
SKILL_DIR=~/.hermes/skills/guizang-ppt-skill
mkdir -p $SKILL_DIR/{assets,references}
BASE="https://raw.githubusercontent.com/op7418/guizang-ppt-skill/main"
curl -sL "$BASE/SKILL.md" -o $SKILL_DIR/SKILL.md
curl -sL "$BASE/assets/template.html" -o $SKILL_DIR/assets/template.html
curl -sL "$BASE/assets/template-swiss.html" -o $SKILL_DIR/assets/template-swiss.html
curl -sL "$BASE/assets/motion.min.js" -o $SKILL_DIR/assets/motion.min.js
for f in themes.md layouts.md checklist.md components.md image-prompts.md screenshot-framing.md themes-swiss.md layouts-swiss.md swiss-layout-lock.md swiss-map-component.md; do
  curl -sL "$BASE/references/$f" -o $SKILL_DIR/references/$f
done
```
...
- **单文件 HTML**：模板是单文件 HTML，所有 CSS/JS 内联。生成报告时不要拆分文件。
